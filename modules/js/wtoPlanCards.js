...
define(["dojo", "dojo/_base/declare",], function (dojo, declare) {
  return declare("welcometo.planCards", null, {
/****************************************
 ...
 */
    ...
    constructor(gamedatas, pId) {
      ...
      gamedatas.planCards.forEach(plan => {
        ...
        var div = dojo.place(globalThis.gameui.format_block('jstpl_planCard', plan), 'plan-cards-container-resizable');
        dojo.connect($('plan-card-' + plan.id), 'click', () => this.onClickPlan(plan.id));

        let desc = plan.desc.map(t => _(t)).join("\n");
        globalThis.gameui.addTooltip(div.id, desc, '');
      });
    },
    ...
    updateValidationMarks(){
      this._validations.forEach((validations, i) => {
        ...
        if(high.length > 0){
          ...
          globalThis.gameui.addTooltip("plan-card-" + this._planIds[i] + "-0", textHigh, "");
        ...
        if(low.length > 0){
          ...
          globalThis.gameui.addTooltip("plan-card-" + this._planIds[i] + "-1", textLow, "");
        }
    ...
    validateCurrentPlayerPlan(planId, validation, animation){
      ...
      dojo.place(globalThis.gameui.format_block("jstpl_scribbleCheckMark", scribble), "plan-card-" + planId + "-validation");
